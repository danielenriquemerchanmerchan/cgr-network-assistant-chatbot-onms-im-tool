"""
bot/handlers/normalizado.py
---------------------------
Handler del comando /normalizado.

La cuadrilla reporta que el servicio fue restablecido. Cambia:
- fase_operativa: cualquiera -> 'normalizado'

Tipo de interaccion: fase_normalizado (genera worklog en MAXIMO).
"""
...
async def handle(update, context):
    ...
    logger.info(f"/normalizado recibido en chat {chat_id} ({chat_type})")

    if chat_type not in ("group", "supergroup"):
        await update.message.reply_text(
            "ℹ️ El comando /normalizado solo funciona en el grupo de la cuadrilla."
        )
        return
    ...
    ok = actualizar_fase_operativa(
        ...
        nueva_fase="normalizado",
    )

    if not ok:
        await update.message.reply_text("❌ No se pudo registrar la normalización.")
        logger.error(f"Fallo actualizar_fase para asignacion {ot['asignacion_id']}")
        return

    registrar_interaccion(
        tipo_interaccion="fase_normalizado",
        direccion="entrante",
        ...
        contenido_texto="/normalizado",
        metadata={
            "comando": "/normalizado",
            "fase_anterior": fase_anterior,
            "fase_nueva": "normalizado",
        },
    )

    await update.message.reply_text(
        f"🟢 Servicio confirmado en *OT {ot['wonum']}*\n\n"
        f"Si NOC pide monitoreo, permanezcan en sitio. "
        f"Cuando se retiren, escriban /retiro.",
        parse_mode="Markdown",
    )

    logger.info(
        f"Normalizacion registrada: {cuadrilla['cuadrilla_id']} en OT {ot['wonum']} "
        f"(de {fase_anterior} a normalizado)"
    )