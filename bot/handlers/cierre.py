...
def _declarar_cierre(asignacion_id):
    """
    Cambia el estado de la asignacion a 'declarada_terminada',
    fija fase_operativa = 'validacion' (esperando CGR),
    y registra fecha_declaracion_cierre.
    """
    conn = None
    try:
        ...
        logger.error(f"Error declarando cierre: {e}")
        if conn:
            conn.rollback()
        return False
    finally:
...
async def handle(update, context):
    ...
    # Cambiar estado a declarada_terminada
    ok = _declarar_cierre(ot["asignacion_id"])
    if not ok:
    ...
    await update.message.reply_text(
        ...
        f"_CGR validará el cierre en breve. Mantengan el sitio hasta confirmación._",
        parse_mode="Markdown",
    )

    logger.info(
        f"Cierre declarado: {cuadrilla['cuadrilla_id']} en OT {ot['wonum']} "
        f"(fase al cierre: {ot['fase_operativa']})"
    )