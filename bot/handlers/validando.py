"""
bot/handlers/validando.py
-------------------------
Handler del comando /validando.

La cuadrilla reporta que esta validando con plataformas/transmision. Cambia:
- fase_operativa: cualquiera -> 'validacion'

Tipo de interaccion: fase_validacion (genera worklog en MAXIMO).
"""
...
async def handle(update, context):
    ...
    logger.info(f"/validando recibido en chat {chat_id} ({chat_type})")

    if chat_type not in ("group", "supergroup"):
        await update.message.reply_text(
            "ℹ️ El comando /validando solo funciona en el grupo de la cuadrilla."
        )
        return
    ...
    ok = actualizar_fase_operativa(
        ...
        nueva_fase="validacion",
    )

    if not ok:
        await update.message.reply_text("❌ No se pudo registrar la validación.")
        logger.error(f"Fallo actualizar_fase para asignacion {ot['asignacion_id']}")
        return

    registrar_interaccion(
        tipo_interaccion="fase_validacion",
        direccion="entrante",
        ...
        contenido_texto="/validando",
        metadata={
            "comando": "/validando",
            "fase_anterior": fase_anterior,
            "fase_nueva": "validacion",
        },
    )

    await update.message.reply_text(
        f"✅ Validación iniciada en *OT {ot['wonum']}*\n\n"
        f"Cuando confirmen servicio restablecido, escriban /normalizado.",
        parse_mode="Markdown",
    )

    logger.info(
        f"Validacion registrada: {cuadrilla['cuadrilla_id']} en OT {ot['wonum']} "
        f"(de {fase_anterior} a validacion)"
    )