"""
bot/handlers/empalmando.py
--------------------------
Handler del comando /empalmando.

La cuadrilla reporta que iniciaron empalme/fusion. Cambia:
- fase_operativa: cualquiera -> 'empalmeria'

Tipo de interaccion: fase_empalmeria (genera worklog en MAXIMO).
"""
...
async def handle(update, context):
    ...
    logger.info(f"/empalmando recibido en chat {chat_id} ({chat_type})")

    if chat_type not in ("group", "supergroup"):
        await update.message.reply_text(
            "ℹ️ El comando /empalmando solo funciona en el grupo de la cuadrilla."
        )
        return
    ...
    ok = actualizar_fase_operativa(
        ...
        nueva_fase="empalmeria",
    )

    if not ok:
        await update.message.reply_text("❌ No se pudo registrar el empalme.")
        logger.error(f"Fallo actualizar_fase para asignacion {ot['asignacion_id']}")
        return

    registrar_interaccion(
        tipo_interaccion="fase_empalmeria",
        direccion="entrante",
        ...
        contenido_texto="/empalmando",
        metadata={
            "comando": "/empalmando",
            "fase_anterior": fase_anterior,
            "fase_nueva": "empalmeria",
        },
    )

    await update.message.reply_text(
        f"🛠️ Empalme iniciado en *OT {ot['wonum']}*\n\n"
        f"Cuando terminen empalme y empiecen pruebas, escriban /validando.",
        parse_mode="Markdown",
    )

    logger.info(
        f"Empalme registrado: {cuadrilla['cuadrilla_id']} en OT {ot['wonum']} "
        f"(de {fase_anterior} a empalmeria)"
    )