...
database.py
-----------
Módulo de operaciones con la base de datos MySQL del bot.

Funciones:
    Técnicos:
        registrar_tecnico(chat_id, nombre, empleado, username, departamento)
        aprobar_tecnico(chat_id)
        rechazar_tecnico(chat_id)
        obtener_tecnicos_activos(departamento)
        obtener_tecnico(chat_id)
        tecnico_existe(chat_id)

    OTs notificadas:
        registrar_ot(ot_data)
        ot_ya_notificada(wonum)
        actualizar_status_ot(wonum, status_nuevo, status_descripcion)
        cerrar_ot(wonum)
        obtener_ots_activas()
        actualizar_ultimo_chequeo(wonum)

    Historial de estados:
        registrar_cambio_estado(wonum, status_anterior, status_nuevo, status_descripcion, fecha_cambio)

    Acuses de recibo:
        registrar_acuse_pendiente(wonum, chat_id, nombre_tecnico, departamento)
        confirmar_acuse(wonum, chat_id)
        obtener_acuses_pendientes(timeout_minutos)

    Avances:
        registrar_avance(wonum, chat_id, nombre_tecnico, texto, exitoso)
"""
