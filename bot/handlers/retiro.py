"""
bot/handlers/retiro.py
----------------------
Handler del comando /retiro.

La cuadrilla reporta que se esta retirando del sitio. Cambia:
- fase_operativa: cualquiera -> 'retiro'

Tipo de interaccion: fase_retiro (genera worklog en MAXIMO).
"""
...
async def handle(update, context):
    ...
    logger.info(f"/retiro recibido en chat {chat_id} ({chat_type})")

    if chat_type not in ("group", "supergroup"):
        await update.message.reply_text(
            "ℹ️ El comando /retiro solo funciona en el grupo de la cuadrilla."
        )
        return
    ...
    if not ok:
        await update.message.reply_text("❌ No se pudo registrar el retiro.")
        logger.error(f"Fallo actualizar_fase para asignacion {ot['asignacion_id']}")
        return

    registrar_interaccion(
        tipo_interaccion="fase_retiro",
        direccion="entrante",
    ...
    await update.message.reply_text(
        f"🚪 Retiro registrado en *OT {ot['wonum']}*\n\n"
        f"Para finalizar el trabajo, escriban /cierre.",
        parse_mode="Markdown",
    )

    logger.info(
        f"Retiro registrado: {cuadrilla['cuadrilla_id']} en OT {ot['wonum']} "
        f"(de {fase_anterior} a retiro)"
    )